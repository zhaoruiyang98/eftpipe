...
class FFTLog(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.dx = log(self.xmax / self.xmin) / (self.Nmax - 1.0)
        self.setx()
        self.setPow()
        self.setCoefFactor()
...
class FFTLog2D:
    ...
    def __init__(
        ...
        self.xPow: NDArray = self.xbias + 2 * pi * 1j * np.fft.fftfreq(
            self.Nxmax, d=self.dx
        )
        self.yPow: NDArray = self.ybias + 2 * pi * 1j * np.fft.fftfreq(
            self.Nymax, d=self.dy
        )
    ...
    def spherical_transform(
        ...
        *,
        ...
        M1 = np.power(k1grid, xpowgrid) * self.bessel_matrix_function(
            self.xPow, l1
        )  # p, m broadcasting
        k2grid, ypowgrid = k2[:, newaxis], (-3.0 - self.yPow)[newaxis, :]
        M2 = np.power(k2grid, ypowgrid) * self.bessel_matrix_function(
            self.yPow, l2
        )  # q, n broadcasting
        return np.real(np.einsum("mn,pm,qn->pq", Coef, M1, M2, optimize=True))

    def bessel_matrix_function(self, p: complex | NDArray, l: int) -> complex | NDArray:
