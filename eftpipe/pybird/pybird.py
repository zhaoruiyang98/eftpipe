...
class FFTLog2D:
    ...
    def __init__(self, **kwargs: float) -> None:
        Nxmax, Nymax = self._same_pop(kwargs, 'Nxmax', 'Nymax')
        if (not isinstance(Nxmax, int)) or (not isinstance(Nymax, int)):
            raise TypeError("'Nxmax' and 'Nymax' should be integers")
        self.Nxmax, self.Nymax = Nxmax, Nymax

        self.xmin, self.xmax = kwargs.pop('xmin'), kwargs.pop('xmax')
        self.ymin, self.ymax = kwargs.pop('ymin'), kwargs.pop('ymax')
        self.xbias, self.ybias = self._same_pop(kwargs, 'xbias', 'ybias')
        if kwargs:
            raise TypeError(f'unexpected keywords {kwargs}')

        self.dx = log(self.xmax / self.xmin) / (self.Nxmax - 1)
        self.dy = log(self.ymax / self.ymin) / (self.Nymax - 1)

        self.x = np.geomspace(self.xmin, self.xmax, self.Nxmax, dtype=np.float64)
        self.y = np.geomspace(self.ymin, self.ymax, self.Nymax, dtype=np.float64)
        self.xPow = self.xbias + 2 * pi * 1j * np.fft.fftfreq(self.Nxmax, d=self.dx)
        self.yPow = self.ybias + 2 * pi * 1j * np.fft.fftfreq(self.Nymax, d=self.dy)

    def Coef(self, xin: np.ndarray, yin: np.ndarray, zin: np.ndarray,
        extrap: str ='padding', nf: Optional[int] = None) -> np.ndarray:
        ...
        out = 1 / (self.Nxmax * self.Nymax) * np.fft.fft2(
            farr * np.outer(
    ...
    def spherical_transform(self, xin: np.ndarray, yin: np.ndarray, zin: np.ndarray,
        extrap: str = 'padding', nf: Optional[int] = None, *, k1: np.ndarray, k2: np.ndarray,
        l1: int, l2: int) -> np.ndarray:

        Coef = self.Coef(xin, yin, zin, extrap=extrap, nf=nf) # m, n
        k1grid, xpowgrid = np.meshgrid(k1, -3.0 - self.xPow, indexing='ij') 
        M1 = np.power(k1grid, xpowgrid) * self.bessel_matrix_function(self.xPow, l1) # p, m broadcast
        k2grid, ypowgrid = np.meshgrid(k2, -3.0 - self.yPow, indexing='ij')
        M2 = np.power(k2grid, ypowgrid) * self.bessel_matrix_function(self.yPow, l2) # q, n broadcast
        return np.real(np.einsum('mn,pm,qn->pq', Coef, M1, M2, optimize=True))
    
    def bessel_matrix_function(self, p: Union[np.ndarray, complex], 
        l: int) -> Union[np.ndarray, complex]:
        r"""helper function to do bessel transfrom

        Parameters
        ----------
        p : Union[np.ndarray, complex]
            power
        l : int
        ...
        """
        out = exp(loggamma(0.5 * (3.0 + l + p)) - loggamma(0.5 * (l - p)))
        return out * np.power(2., 1. + p) * sqrt(pi)
    
    def window(self, nf: int) -> np.ndarray:
        fx = np.fft.fftfreq(self.Nxmax)
        fy = np.fft.fftfreq(self.Nymax)
        ...
        wx[fx < fx_left] = thetax_left - 1 / (2 * pi) * sin(2 * pi * thetax_left)
        wx[fx > fx_right] = thetax_right - 1 / (2 * pi) * sin(2 * pi * thetax_right)
        wy[fy < fy_left] = thetay_left - 1 / (2 * pi) * sin(2 * pi * thetay_left)
        wy[fy > fy_right] = thetay_right - 1 / (2 * pi) * sin(2 * pi * thetay_right)

        return np.outer(wx, wy)
        
    def _same_pop(self, dic: Dict[Any, Any], key1: Any, key2: Any) -> Tuple[Any, Any]:
        x, y = dic.pop(key1, None), dic.pop(key2, None)
        if (x is None) and (y is None):
            raise KeyError(f"Please specify keyword <{key1!r}> or <{key2!r}> or both")
...
class NonLinear(object):
    ...
    def __init__(self, load=True, save=True, path='./', NFFT=256, co=common):
        ...
        if save is True:
            try:
                np.savez(os.path.join(path, f'pyegg{NFFT}_Nl{co.Nl}.npz'), Pow=self.fft.Pow,
...
class Resum(object):
    ...
    def __init__(self, LambdaIR=1., NFFT=192, co: Common = common):
        ...
        self.IRloop = np.zeros(shape=(self.co.Nl, self.co.Nloop, self.co.Nn, self.co.Nk))
...
class Window(HasLogger):
    ...
    def __init__(
        ...
        self.window_fourier_file = \
            Path(window_fourier_file).resolve()\
            if window_fourier_file else None
        self.window_configspace_file = \
            Path(window_configspace_file).resolve()\
            if window_configspace_file else None
        ...
        self.meta = dict(
            ...
            Nmax=Nmax, xmin_factor=xmin_factor, xmax_factor=xmax_factor, bias=bias,
            ...
            try:
                ...
                if Wal.shape[1] != self.meta["Nl"]:
                    ...
                    try:
                        ...
                        self.mpi_warning(
                            "Cannot load mask: %s", window_fourier_file)
    ...
    def _compute_Wal(self):
        ...
        try:
            ...
            raise OSError(
                f"Error: can't load mask file: "
                f"{self.window_configspace_file}"
            )
        except IndexError as ex:
            raise TypeError(
                f'loaded config-space mask has unexpected shape'
            ) from ex
        ...
        self.fft = FFTLog(**self.fftsettings)
        self.pPow = exp(
            np.einsum('n,p->np', -self.fft.Pow - 3., log(self.p)))
        self.M = np.empty(
            shape=(Nl, self.fft.Pow.shape[0]), dtype='complex')
        for l in range(Nl):
    ...
    def _compute_Waldk(self):
        ...
        if self.withmask:
            ...
            mask = (kpgrid < kgrid + self.windowk)\
                & (kpgrid > kgrid - self.windowk)
